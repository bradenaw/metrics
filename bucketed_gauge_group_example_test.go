...
// Server is an HTTP/2 server that manages multiple streams per TCP connection.
type Server struct {
...
func NewServer(mtr *metrics.Metrics) *Server {
	s := &Server{ /* ... */ }
...
func (s *Server) Close() {
	s.stopEmittingMetrics()
}

// --- main.go -------------------------------------------------------------------------------------
func Example_BucketedGaugeGroup() {
}
